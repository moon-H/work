 ...
 */
@SuppressLint("NewApi")
public class CenterManagerCommon {
    ...
    public CenterManagerCommon(Context context) {
        mContext = context;
    ...
    public class LocalBinder extends Binder {
        public CenterManagerCommon getService() {
            return CenterManagerCommon.this;
        }
    }
    ...
    ScanCallback mScanCallback = new ScanCallback() {
        ...
        public void onScanResult(int callbackType, ScanResult result) {
            ...
            sendBleBroadCast(ACTION_FIND_TARGET_DEVICE, result);

            //            if (DataContainer.bleScanResultList.size() == 0)
            //                DataContainer.bleScanResultList.add(result);
            //            else {
            //                //扫描到新设备
            //            boolean isContainedDevice = false;
            //            ScanRecord currentRecord = result.getScanRecord();
            //            for (ScanResult tempScanResult : DataContainer.bleScanResultList) {
            //                if (currentRecord != null && tempScanResult.getScanRecord() != null) {
            //                    Map<ParcelUuid, byte[]> currentDataMap = currentRecord.getServiceData();
            //                    byte[] currentData = currentDataMap.get(new ParcelUuid(Common.UUID_SERVICE_ENTRY_GATE));
            //                    byte[] savedData = tempScanResult.getScanRecord().getServiceData().get(new ParcelUuid(Common.UUID_SERVICE_ENTRY_GATE));
            //                    MLog.d(TAG, " currentData = " + HexConverter.bytesToHexString(currentData));
            //                    MLog.d(TAG, " savedData = " + HexConverter.bytesToHexString(savedData));
    ...
    public void startScanLeDevice(UUID uuid) {
    ...
    private BluetoothAdapter.LeScanCallback mLeScanCallback = new BluetoothAdapter.LeScanCallback() {
        ...
        public void onLeScan(final BluetoothDevice device, final int rssi, final byte[] scanRecord) {
            ...
            DataContainer.oldApiFindDevice = device;
        }
    };

    public void stopScanOldApi() {
